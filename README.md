...
**版本**：0.2.2  
**开发语言**：Python 3.10+  
**核心依赖**：Bismark ≥ 0.24.0, Bowtie2 ≥ 2.5.0, SAMtools ≥ 1.15  
**v0.2.2 更新**：NCBI WT 4 个 SRR 样本完成分析；5 个 DRR 样本因比对率极低标记为失败（详见故障排除）；NC2M 三样本完整差异分析报告  
...
### 实测验证: Neurospora crassa NC2M 样本 (三样本完整验证)

使用 Neurospora crassa 野生型 WGBS 数据 (NC2M_1/2/3, NC12 参考基因组) 验证软件输出与商业公司结果的一致性。详细分析见 `wgbs_tools/NC2M_差异分析报告.md`。
...
| **甲基化组成偏差 (mCG/mCHG/mCHH)** | <0.5% | <0.5% | <0.5% |

**关键结论**：
- ✅ WGBS_tools 成功覆盖 >98.5% 的公司检测位点，远超 <5% 差异目标
- ✅ 共同位点甲基化水平差异极小（平均绝对差异 ~0.009，<1 个百分点）
- ✅ 甲基化组成比例与公司结果偏差 <0.5%
- 📌 额外检测 12-18% 位点来自 Bowtie2 2.5.4 比对灵敏度提升，多为低水平甲基化位点（中位 Level ~4%）

### NCBI 野生型样本分析结果

| 类别 | 样本数 | 状态 | 说明 |
|------|--------|------|------|
| **SRR 样本** | 4 | ✅ 完成 | SRR1566116, SRR3106959, SRR3106960, SRR3476867; 比对率 80%+, 位点数 0.6-1.1M |
| **DRR 样本** | 5 | 🔴 失败 | DRR001162-166; 比对率 0-4%, 原因详见问题 6 |
...
### 问题 6: DRR 样本 (DDBJ) 比对率极低 — 数据不兼容 🔴

**现象**: 5 个 DRR 系列样本 (DRR001162–DRR001166) 在 Neurospora crassa NC12 参考基因组上的 Bismark 比对率极低 (0–4%)，而同期处理的 4 个 SRR 样本比对率正常 (80%+)。

**各样本详细诊断**:

| 样本 | 原始 FASTQ | 比对率 | BAM 大小 | 位点产出 | 根因 |
|------|-----------|--------|----------|----------|------|
| DRR001162 | 3.9G | 3.5% | 192M | 109,124 | 非 N. crassa 数据或严重污染 |
| DRR001163 | 561M | 3.9% | 32M | 24,332 | 同上 |
| DRR001164 | 397M | 2.6% | 13M | 13,276 | 同上 |
| DRR001165 | 1.5G | **0.04%** | 601K | 767 | Reads 5' 端 5×N 碱基 + 非目标物种 |
| DRR001166 | 4.4G | **0.007%** | 272K | 0 | Reads 质量极差 (Q≈2), 大量 N 填充 |

**原因分析**:

1. **物种不匹配 (主因)**：DRR 样本来自日本 DDBJ 序列档案馆。下载时按 Neurospora crassa WGBS 关键词检索，但这些 accession 的原始提交可能标记有误，或实际为非 N. crassa 物种的数据。比对率 0-4% vs 预期的 80%+ 强烈支持物种不匹配的诊断。

2. **DRR001166 数据质量灾难性损坏**：Reads 质量分全部为 `#` (ASCII 35, Q≈2)，且序列由大量 N 填充：
   ```
   @DRR001166.1 HWUSI-EAS1548_24:3:1:1183:946 length=121
   NAATTATATTTAATTATATAAATTAAAAATNNNNNNNNNNNNNNNNNNNNNNNNNNN...
   +#####################################################################################
   ```
   这表明原始测序数据在存档或传输过程中已严重损坏。

3. **DRR001165 reads 5' 端 N 碱基**：Reads 前 5 bp 均为 N (被 trim 或质量过滤)，与 Bismark 的 `--ignore 5` 参数叠加导致有效序列过短，进一步降低比对成功率。

4. **甲基化模式异常**：即使少数比对的 reads，甲基化模式也与 Neurospora 特征不符 (DRR 样本 mCG ≈ 6% vs 正常 ~21%, mCHH ≈ 23% vs 正常 ~71%)。

**结论**: 5 个 DRR 样本**不可用于下游分析**。仅保留 4 个 SRR 样本的结果作为 NCBI WT 的有效产出。

**教训与建议**:
- 下载公共 SRA 数据前，应先在 NCBI SRA Run Selector 中确认每个 accession 的物种、文库类型和测序平台信息
- DDBJ (日本) 和 ENA (欧洲) 的 accession 可能存在元数据不一致，需额外核对
- 批次分析中应增加比对率检查（如比对率 <20% 自动标记为异常并跳过后续步骤）
- 本软件的下一个版本将加入自动比对率检查和异常样本标记功能
